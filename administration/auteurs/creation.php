...
if ($formulaire_soumis) {
    ...
    if ($nom === "" || $prenom === "" || $lien_avatar === "") {
        ...
        $requete_brute = "
            ...
            VALUES ('$prenom_sql', '$nom_sql', '$lien_avatar_sql', '$lien_twitter_sql')
        ";
        ...
        if ($resultat_brut) {
            ...
            $erreur = "Erreur lors de la création de l'auteur : " . mysqli_error($mysqli_link);
        }
    }
}
?>
...
<head>
    ...
    <title>Créer un auteur - Administration</title>
</head>
...
<body>
    ...
    <header class="bg-white shadow">
        <div class="mx-auto max-w-7xl py-3 px-4">
            <p class="text-3xl font-bold text-gray-900">Créer un nouvel auteur</p>
        </div>
    </header>
                    ...
                    <section class="grid grid-cols-1 gap-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="nom" id="nom" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 border p-2">
                            </div>
                            <div>
                                <label for="prenom" class="block text-lg font-medium text-gray-700">Prénom *</label>
                                <input type="text" name="prenom" id="prenom" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 border p-2">
                            </div>
                        </div>

                        <div>
                            <label for="avatar" class="block text-lg font-medium text-gray-700">Lien avatar *</label>
                            <input type="url" name="lien_avatar" id="avatar" required placeholder="https://..." class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 border p-2">
                            <p class="text-sm text-gray-500 mt-1">URL vers une image (ex: portrait)</p>
                        </div>

                        <div>
                            <label for="lien_twitter" class="block text-lg font-medium text-gray-700">Lien Twitter</label>
                            <input type="url" name="lien_twitter" id="lien_twitter" placeholder="https://twitter.com/..." class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 border p-2">
                        </div>
